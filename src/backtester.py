...
class BasisBacktester:
    ...
    def _preprocess(self):
        ...
        spot_dvds = self.resample_series(spot[self.dividend_col], self.interval, 'sum').fillna(0)
        spot_cum_dvds = spot_dvds.cumsum() # Cumulative dividends

        df = pd.DataFrame({
            'spot': spot,
            'futures': fut,
            'dividends': spot_cum_dvds
        }).dropna()
    ...
    def run_backtest(
        ...
        for t, sig in signal.items():
            ...
            if spot_trade.get('size'):
                spot_trade['transaction_cost'] = spot_trade['price'] * spot_trade['size'] * commission_rate
                fut_trade['transaction_cost'] = fut_trade['price'] * fut_trade['size'] * commission_rate
    ...
    def analyze_results(self):
        ...
        total_pnl = (sign * (df['price'] + df['dividends']) * df['size'] - df['transaction_cost']).cumsum()
        ...
        for t, row in df[df['instrument'] == 'spot'].iterrows():
            if not spot_trades or spot_trades[-1][1]== row['direction']:
                ...
                holding_time.append(
                    (t - last_trade[0]).total_seconds() / 60
                )
                if last_trade_size < row['size']:
                    trade_sizes.append(last_trade_size)
                    spot_trades.append((t, row['direction'], row['size']-last_trade_size))
                elif last_trade_size > row['size']:
                    trade_sizes.append(row['size'])
                    spot_trades.append((last_trade[0], last_trade[1], last_trade_size-row['size']))
                else:
                    trade_sizes.append(row['size'])

        avg_holding_time = np.average(holding_time, weights=trade_sizes) # Size-weighted average holding time
