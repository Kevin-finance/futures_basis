...
def plot_spreads(df):
    ...
    fig.add_trace(go.Scatter(x=ts, y=df["Theoretical Basis"].to_numpy(), name="Theo - Spot"))
    fig.add_trace(go.Scatter(x=ts, y=df["BTIC"].to_numpy(), name="BTIC", line=dict(dash="dot")))
    fig.update_layout(title="Spread to Spot", template="plotly_white", hovermode="x unified")
...
def plot_percentage_changes(df:pl.DataFrame):
    ...
    ts = df["UTC-Datetime"].to_numpy()
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=ts, y=((df["Market Basis"] - es0)/es0*100), name="ES % Δ"))
    fig.add_trace(go.Scatter(x=ts, y=((df["Theoretical Basis"] - theo0)/theo0*100), name="Theo % Δ"))
    fig.add_trace(go.Scatter(x=ts, y=((df["BTIC"] - btic0)/btic0*100), name="BTIC % Δ", line=dict(dash="dot")))
    fig.update_layout(title="% Change (Normalized)", template="plotly_white", hovermode="x unified")
...
def plot_twinx(df: pl.DataFrame):
    ...
    fig.add_trace(go.Scatter(x=ts, y=btic, mode="lines", name="BTIC", line=dict(dash="dot")), secondary_y=True)
...
def plot_twinx_for_date(df, date_str):
    ...
    fig = make_subplots(specs=[[{"secondary_y": True}]])
    fig.add_trace(go.Scatter(x=ts, y=subset["Market Basis"].to_numpy(), name="ES - Spot"), secondary_y=False)
    fig.add_trace(go.Scatter(x=ts, y=subset["Theoretical Basis"].to_numpy(), name="Theo - Spot"), secondary_y=False)
    fig.add_trace(go.Scatter(x=ts, y=subset["BTIC"].to_numpy(), name="BTIC", line=dict(dash="dot")), secondary_y=True)
    fig.update_layout(title=f"Twinx Plot on {date_str}", hovermode="x unified", template="plotly_white")
